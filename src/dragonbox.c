...
// Checked out at beeeef91cf6fef89a4d4ba5e95d47ca64ccb3a44
...
#if !defined(uint_least64_t)
#define uint_least64_t uint64_t
#endif

static const size_t detail_physical_bits_value_u64 = sizeof(uint64_t)*8; // total size in bits
static const size_t detail_value_bits_bits_value_u64 = 64; // size in bits without signing bit

#define bit_cast(from, to, a) ((to)a)
 ...
 struct ieee754_binary_t {
    ...
    const int decimal_significand_digits;
    const int decimal_exponent_digits;
    const int max_abs_decimal_exponent;
};

static const struct ieee754_binary_t ieee754_binary64 = {
    .total_bits = 64,
    .significand_bits = 52,
    .exponent_bits = 11,
    .min_exponent = -1022,
...
static inline const int extract_exponent_bits(uint64_t u) {
    return (u >> ieee754_binary64.significand_bits) &
           ((1 << ieee754_binary64.exponent_bits) - 1);
}

// Extract significand bits from a bit pattern.
// The result must be aligned to the LSB so that there is no additional zero paddings
// on the right. The result does not contain the implicit bit.
static inline const uint64_t extract_significand_bits(uint64_t u) {
    return u & ((UINT64_C(1) << ieee754_binary64.significand_bits) - 1u);
}

// Remove the exponent bits and extract significand bits together with the sign bit.
static inline const uint64_t remove_exponent_bits(uint64_t u) {
    return u & ~(((UINT64_C(1) << ieee754_binary64.exponent_bits) - 1u)
                 << ieee754_binary64.significand_bits);
}

// Shift the obtained signed significand bits to the left by 1 to remove the sign bit.
static inline const uint64_t remove_sign_bit_and_shift(uint64_t u) {
    return (u << 1) &
           ((((UINT64_C(1) << (ieee754_binary64.total_bits - 1)) - 1u) << 1) | 1u);
}

// Obtain the actual value of the binary exponent from the extracted exponent bits.
static inline const int binary_exponent(int exponent_bits) {
    return exponent_bits == 0 ? ieee754_binary64.min_exponent
                              : exponent_bits + ieee754_binary64.exponent_bias;
}

// Obtain the actual value of the binary significand from the extracted significand bits
// and exponent bits.
static inline const uint64_t binary_significand(uint64_t significand_bits, int exponent_bits) {
    return exponent_bits == 0
               ? significand_bits
               : (significand_bits | (UINT64_C(1) << ieee754_binary64.significand_bits));
}

/* Various boolean observer functions */

static inline const bool is_nonzero(uint64_t u) {
    return (u & ((UINT64_C(1) << (ieee754_binary64.significand_bits + ieee754_binary64.exponent_bits)) -
                    1u)) != 0;
}
static inline const bool is_positive(uint64_t u) {
    return u < (UINT64_C(1) << (ieee754_binary64.significand_bits + ieee754_binary64.exponent_bits));
}
static inline const bool is_negative(uint64_t u) { return !is_positive(u); }
static inline const bool is_finite(int exponent_bits) {
    return exponent_bits != ((INT32_C(1) << ieee754_binary64.exponent_bits) - 1);
}
static inline const bool has_all_zero_significand_bits(uint64_t u) {
    return ((u << 1) &
            ((((UINT64_C(1) << (ieee754_binary64.total_bits - 1)) - 1u) << 1) | 1u)) == 0;
}
static inline const bool has_even_significand_bits(uint64_t u) {
    return u % 2 == 0;
}
/////////////////////////////////////////////////////////////////////////////////////////////////
...
/////////////////////////////////////////////////////////////////////////////////////////////////
// Convert between bit patterns stored in uint64_t and instances of an actual
// floating-point type. Depending on format and uint64_t, this operation might not
// be possible for some specific bit patterns. However, the contract is that u always
// denotes a valid bit pattern, so the functions here are assumed to be.
...
static inline const uint64_t float_to_carrier(const double x) {
    return bit_cast(double, uint64_t, x);
}

// Converts the bit-carrier unsigned integer type into the floating-point type.
static inline const double carrier_to_float(const uint64_t x) {
    return bit_cast(double, uint64_t, x);
}
/////////////////////////////////////////////////////////////////////////////////////////////////
...
//
typedef uint64_t signed_significand_bits;
typedef uint64_t float_bits;
typedef int exponent_int;

// struct float_bits {
// const exponent_int binary_exponent() const {
//         return binary_exponent(extract_exponent_bits());
//     }


static inline const float_bits make_float_bits(double x) {
    return float_to_carrier(x);
}
/////////////////////////////////////////////////////////////////////////////////////////////////
...
////////////////////////////////////////////////////////////////////////////////////////
static inline uint64_t rotr_u64_64(uint64_t n, unsigned int r) {
    const size_t bit_width = 64;
    r &= (bit_width - 1);
    return (n >> r) | (n << ((bit_width - r) & (bit_width - 1)));
}

static inline uint64_t rotr_u64_32(uint64_t n, unsigned int r) {
    const size_t bit_width = 32;
    r &= (bit_width - 1);
    return (n >> r) | (n << ((bit_width - r) & (bit_width - 1)));
}

static inline uint32_t rotr_u32_32(uint32_t n, unsigned int r) {
    const size_t bit_width = 32;
    r &= (bit_width - 1);
    return (n >> r) | (n << ((bit_width - r) & (bit_width - 1)));
}
...
// [-Wpedantic]"
                ...
                __extension__
#endif
typedef builtin_uint128_t unsigned __int128;
#endif
// clang-format on
...
void operator_add_eq(uint128 *v, uint_least64_t n) {
// See https://github.com/fmtlib/fmt/pull/2985.
#if JKJ_HAS_BUILTIN(__builtin_addcll) && !defined(__ibmxl__)
#if __UINT_LEAST64_MAX == ULONG_LONG_MAX
    unsigned long long carry;
    v->low_ = (uint_least64_t)(__builtin_addcll(v->low_, n, 0, &carry));
    v->high_ = (uint_least64_t)(__builtin_addcll(v->high_, 0, carry, &carry));
    return v;
#endif
#endif
#if JKJ_HAS_BUILTIN(__builtin_addcl) && !defined(__ibmxl__)
#if __UINT_LEAST64_MAX == ULONG_MAX
    unsigned long carry;
    v->low_ = (uint_least64_t)(
        __builtin_addcl((unsigned long)(v->low_),
                        (unsigned long)(n), 0, &carry));
    v->high_ = (uint_least64_t)(
        __builtin_addcl((unsigned long)(v->high_), 0, carry, &carry));
    return v;
#endif
#endif
#if JKJ_HAS_BUILTIN(__builtin_addc) && !defined(__ibmxl__)
#if __UINT_LEAST64_MAX == UINT_MAX
    unsigned int carry;
    v->low_ = (uint_least64_t)(__builtin_addc((unsigned int)(v->low_),
                                                (unsigned int)(n), 0,
                                                &carry));
    v->high_ = (uint_least64_t)(
        __builtin_addc((unsigned int)(v->high_), 0, carry, &carry));
    return v;
#endif
#endif
    ...
    const unsigned char carry = __builtin_ia32_addcarry_u64(0, v->low_, n, &result);
    v->low_ = (uint_least64_t)(result);
    __builtin_ia32_addcarry_u64(carry, v->high_, 0, &result);
    v->high_ = (uint_least64_t)(result);
    ...
    static_assert(sizeof(unsigned long long) == sizeof(uint_least64_t), "");
    const unsigned char carry = _addcarry_u64(0, v->low_, n, &(v->low_));
    _addcarry_u64(carry, v->high_, 0, &(v->high_));
    ...
    // is always true...
    static_assert(sizeof(unsigned __int64) == sizeof(uint_least64_t), "");
    const unsigned char carry = _addcarry_u64(0, v->low_, n, &(v->low_));
    _addcarry_u64(carry, v->high_, 0, &(v->high_));
#else
    const uint_least64_t sum = (v->low_ + n) & UINT64_C(0xffffffffffffffff);
    v->high_ += (sum < v->low_ ? 1 : 0);
    v->low_ = sum;
#endif
    return v;
}
...
typedef struct {
    uint_least64_t low64_;
    uint_least32_t high32_;
} uint32_64;


typedef struct {
    uint_least64_t high64_;
    uint_least32_t low32_;
} uint64_32;


static inline uint_least64_t umul64(uint_least32_t x, uint_least32_t y) {
...
static inline uint128 umul128_generic(uint_least64_t x, uint_least64_t y) {
        const uint_least32_t a = (uint_least32_t)(x >> 32);
        const uint_least32_t b = (uint_least32_t)(x);
        const uint_least32_t c = (uint_least32_t)(y >> 32);
        const uint_least32_t d = (uint_least32_t)(y);

        const uint_least64_t ac = umul64(a, c);
        const uint_least64_t bc = umul64(b, c);
        const uint_least64_t ad = umul64(a, d);
        const uint_least64_t bd = umul64(b, d);

        const uint_least64_t intermediate =
            (bd >> 32) + (uint_least32_t)(ad) + (uint_least32_t)(bc);

        return (uint128){ac + (intermediate >> 32) + (ad >> 32) + (bc >> 32),
                (intermediate << 32) + (uint_least32_t)(bd)};
    };

// Get 128-bit result of multiplication of two 64-bit unsigned integers.
static inline uint128 umul128(uint_least64_t x, uint_least64_t y) {
#if defined(__SIZEOF_INT128__)
    const builtin_uint128_t result = (builtin_uint128_t)(x) * (builtin_uint128_t)(y);
    return (uint128){(uint_least64_t)(result >> 64), (uint_least64_t)(result)};
#elif defined(_MSC_VER) && defined(_M_X64)
    JKJ_IF_CONSTEVAL {
        // This redundant variable is to workaround MSVC's codegen bug caused by the
        // interaction of NRVO and intrinsics.
        const uint128 result = umul128_generic();
        return result;
    }
    uint128 result;
#if defined(__AVX2__)
    ...
    return result;
#else
    return umul128_generic();
#endif
}

static inline uint_least64_t umul128_upper64_generic(uint_least64_t x, uint_least64_t y) {
    const uint_least32_t a = (uint_least32_t)(x >> 32);
    const uint_least32_t b = (uint_least32_t)(x);
    const uint_least32_t c = (uint_least32_t)(y >> 32);
    const uint_least32_t d = (uint_least32_t)(y);
...
// integers.
static inline uint_least64_t umul128_upper64(uint_least64_t x, uint_least64_t y) {
#if defined(__SIZEOF_INT128__)
    const builtin_uint128_t result = (builtin_uint128_t)(x) * (builtin_uint128_t)(y);
    ...
    JKJ_IF_CONSTEVAL {
        // This redundant variable is to workaround MSVC's codegen bug caused by the
        // interaction of NRVO and intrinsics.
        const uint_least64_t result = umul128_upper64_generic();
        return result;
    }
    ...
    return result;
#else
    return umul128_upper64_generic();
#endif
}

// Get upper 128-bits of multiplication of a 64-bit unsigned integer and a 128-bit
// unsigned integer.
static inline uint128 umul192_upper128(uint_least64_t x, uint128 y) {
...
static inline
    ...
    uint64_32
#endif
    umul96(uint_fast32_t x, uint_least64_t y) {
#if defined(__SIZEOF_INT128__) || (defined(_MSC_VER) && defined(_M_X64))
    const uint128 r128 = umul128((uint_least64_t)(x), y);

    const uint_least32_t high = (uint_least32_t)(r128.high);
    const uint_least32_t middle = (uint_least32_t)(r128.low >> 32);
    const uint_least32_t low = (uint_least32_t)(r128.low);
#else
    const uint_least32_t yh = (uint_least32_t)(y >> 32);
    const uint_least32_t yl = (uint_least32_t)(y);

    uint_least64_t xyh = umul64(x, yh);
    const uint_least64_t xyl = umul64(x, yl);

    xyh += (xyl >> 32);

    const uint_least32_t high = (uint_least32_t)(xyh >> 32);
    const uint_least32_t middle = (uint_least32_t)(xyh);
    const uint_least32_t low = (uint_least32_t)(xyl);
#endif

#if JKJ_FAST_MUL64
    uint32_64 r = {high, ((uint_least64_t)(middle) << 32) | low};
#else
    uint64_32 r = {((uint_least64_t)(high) << 32) | middle, low};
#endif
    return r;
}
...
// unsigned integer.
static inline uint_least64_t umul96_upper64(uint_least32_t x, uint_least64_t y) {
#if defined(__SIZEOF_INT128__) || (defined(_MSC_VER) && defined(_M_X64))
    ...
    const uint_least64_t xyh = umul64(x, yh);
    const uint_least64_t xyl = umul64(x, yl);
...
// unsigned integer.
static inline uint128 umul192_lower128(uint_least64_t x, uint128 y) {
    const uint_least64_t high = x * y.high;
    const uint128 high_low = umul128(x, y.low);
    return {(high + high_low.high) & UINT64_C(0xffffffffffffffff), v->high_low.low};
}

// Get lower 64-bits of multiplication of a 32-bit unsigned integer and a 64-bit
// unsigned integer.
static inline const uint_least64_t umul96_lower64(uint_least32_t x, uint_least64_t y) {
...
static inline int64_t compute_power_i64(int k, int64_t a) {
    int64_t p = 1;
    for (int i = 0; i < k; ++i) {
        p *= a;
    }
...
// Returns -1 when n = 0.
static inline int floor_log2_uint64(uint64_t n) {
    int count = -1;
    while (n != 0) {
        ++count;
        n >>= 1;
...
static inline bool check_divisibility_and_divide_by_pow10_1_u64(uint64_t *n) {
    const uint_fast32_t magic_number = 6554;
    const int shift_amount = 16;
    const uint_fast32_t prod = *n * magic_number;
    const uint_fast32_t mask = ((uint_fast32_t)(1) << shift_amount) - 1;
...
static inline bool check_divisibility_and_divide_by_pow10_1_u8(uint_fast8_t *n) {
    const uint_fast16_t magic_number = 103;
    const int shift_amount = 10;
    const uint_fast16_t prod = *n * magic_number;
    const uint_fast16_t mask = ((uint_fast16_t)(1) << shift_amount) - 1;
    bool const result = ((prod & mask) < magic_number);
...
static inline bool check_divisibility_and_divide_by_pow10_1_u16(uint_least16_t *n) {
    const uint_fast16_t magic_number = 103;
    const int shift_amount = 10;
    const uint_fast16_t prod = *n * magic_number;
    const uint_fast16_t mask = ((uint_fast16_t)(1) << shift_amount) - 1;
...
static inline bool check_divisibility_and_divide_by_pow10_2_u64(uint64_t *n) {
    const uint_fast32_t magic_number = 656;
    const int shift_amount = 16;
    const uint_fast32_t prod = *n * magic_number;
    const uint_fast32_t mask = ((uint_fast32_t)(1) << shift_amount) - 1;
    bool const result = ((prod & mask) < magic_number);
...
static inline bool check_divisibility_and_divide_by_pow10_2_u16(uint_least16_t *n) {
    const uint_fast32_t magic_number = 103;
    const int shift_amount = 10;
    const uint_fast32_t prod = *n * magic_number;
    const uint_fast32_t mask = ((uint_fast32_t)(1) << shift_amount) - 1;
    bool const result = ((prod & mask) < magic_number);
...
// Precondition: n <= 10^(N+1)
static uint64_t small_division_by_pow10_1_u64(uint64_t n) {
    const uint_fast32_t magic_number = 6554;
    const int shift_amount = 16;
    return (n * magic_number) >> shift_amount;
}

static uint8_t small_division_by_pow10_1_u8(uint8_t n) {
    const uint_fast16_t magic_number = 103;
    const int shift_amount = 10;
    return (n * magic_number) >> shift_amount;
}

static uint16_t small_division_by_pow10_1_u16(uint16_t n) {
    const uint_fast16_t magic_number = 103;
    const int shift_amount = 10;
    return (n * magic_number) >> shift_amount;
}

static uint64_t small_division_by_pow10_2_u64(uint64_t n) {
    const uint_fast32_t magic_number = 656;
    const int shift_amount = 16;
    return (n * magic_number) >> shift_amount;
}

static uint_least16_t small_division_by_pow10_2_u16(uint_least16_t n) {
    const uint_fast32_t magic_number = 103;
    const int shift_amount = 10;
    return (n * magic_number) >> shift_amount;
}
...
static inline uint64_t compute_power_1_u64(uint64_t a) {
    int e = 1;
    uint64_t p = 1;
    while (e) {
        if (e % 2)
...
static inline uint64_t compute_power_2_u64(uint64_t a) {
    int e = 2;
    uint64_t p = 1;
    while (e) {
        if (e % 2)
...
static inline uint64_t compute_power_3_u64(uint64_t a) {
    int e = 3;
    uint64_t p = 1;
    while (e) {
        if (e % 2)
...
static inline uint64_t divide_by_pow10_1_u64_32bit(uint64_t n, uint64_t n_max) {
    ...
    if(n_max <= UINT32_C(1073741828)) {
        return umul64(n, UINT32_C(429496730)) >> 32;
    }
    // Specialize for 64-bit division by 10.
    // Without the bound on n_max (which compilers these days never leverage), the
    // minimum needed amount of shift is larger than 64.
    #if UINT64_MAX == UINT_LEAST64_MAX
    if(n_max <= UINT64_C(4611686018427387908)) {
        return umul128_upper64(n, UINT64_C(1844674407370955162));
    }
    #endif

    const uint64_t divisor = compute_power_1_u64(10);
    return n / divisor;
}

static inline uint64_t divide_by_pow10_2_u64_32bit(uint64_t n, uint64_t n_max) {
    ...
    #if UINT64_MAX == UINT_LEAST32_MAX
        return umul64(n, UINT32_C(1374389535)) >> 37;
    #endif

    const uint64_t divisor = compute_power_2_u64(10);
    return n / divisor;
}

static inline uint64_t divide_by_pow10_3_u64_32bit(uint64_t n, uint64_t n_max) {
    // Specialize for 64-bit division by 1000.
    // Without the bound on n_max (which compilers these days never leverage), the
    // smallest magic number for this computation does not fit into 64-bits.
    #if UINT64_MAX == UINT_LEAST64_MAX
    if (n_max <= UINT64_C(15534100272597517998)) {
        return umul128_upper64(n, UINT64_C(4722366482869645214)) >> 8;
    }
    #endif

    const uint64_t divisor = compute_power_3_u64(10);
    return n / divisor;
}
...
typedef struct {
    const int cache_bits;
    const int min_k;
    const int max_k;
    const uint128 cache[619];
} cache_holder_t;

static const cache_holder_t cache_holder_u64 = {
    .cache_bits = 128,
    .min_k = -292,
    .max_k = 326,
    .cache = {
...
static inline builtin_uint128_t get_cache(int k) {
    return cache_holder_u64.cache + k - cache_holder_u64.min_k;
}
...
static inline void remove_trailing_zeros(uint_least64_t *significand, int* exponent) {
    ...
    uint64_t r = rotr_u64_64((uint64_t)(*significand * UINT64_C(28999941890838049)), 8);
    bool b = r < UINT64_C(184467440738);
    int s = b;
    *significand = b ? r : *significand;

    r = rotr_u64_64((uint64_t)(*significand * UINT64_C(182622766329724561)), 4);
    b = r < UINT64_C(1844674407370956);
    s = s * 2 + b;
    *significand = b ? r : *significand;

    r = rotr_u64_64((uint64_t)(*significand * UINT64_C(10330176681277348905)), 2);
    b = r < UINT64_C(184467440737095517);
    s = s * 2 + b;
    *significand = b ? r : *significand;

    r = rotr_u64_64((uint64_t)(*significand * UINT64_C(14757395258967641293)), 1);
...
static inline compute_mul_result compute_mul(uint64_t u, const uint128 *cache) {
    const uint128 r = umul192_upper128(u, *cache);
    return (compute_mul_result){.integer_part = r.high, .is_integer = r.low == 0};
}

static inline uint_least64_t compute_delta(const uint128* cache, int beta) {
    return (uint_least64_t)(cache->high >> (int)(ieee754_binary64.total_bits - 1 - beta));
}

static inline compute_mul_parity_result compute_mul_parity(uint64_t two_f, const uint128 * cache, int beta) {
    const uint128 r = umul192_lower128(two_f, *cache);
    return (compute_mul_parity_result){
        .parity = ((r.high >> (int)(64 - beta)) & 1) != 0,
        .is_integer = (((r.high << beta) & UINT64_C(0xffffffffffffffff)) |
                (r.low >> (int)(64 - beta))) == 0};
}
...
static inline uint64_t compute_left_endpoint_for_shorter_interval_case(const uint128* cache, int beta) {
    return (cache->high - (cache->high >> (ieee754_binary64.significand_bits + 2))) >>
            (int)(ieee754_binary64.total_bits - ieee754_binary64.significand_bits - 1 - beta);
}


static inline uint64_t compute_right_endpoint_for_shorter_interval_case(const uint128* cache,int beta) {
    return (cache->high + (cache->high >> (ieee754_binary64.significand_bits + 1))) >>
            (int)(ieee754_binary64.total_bits - ieee754_binary64.significand_bits - 1 - beta);
}

static inline uint64_t compute_round_up_for_shorter_interval_case(const uint128 *cache,
                                                                  int beta) {
    return ((cache->high >> (int)(ieee754_binary64.total_bits -
                                  ieee754_binary64.significand_bits - 2 - beta)) +
            1) /
           2;
}
...
static const int kappa = 2; // floor_log10_pow2(64 - 52 - 2) - 1;
...
static inline int case_shorter_interval_right_endpoint_lower_threshold = 0;
static inline int case_shorter_interval_right_endpoint_upper_threshold = 3;

static inline int shorter_interval_tie_lower_threshold = -77;
static inline int shorter_interval_tie_upper_threshold = -77;
...
static inline bool check_divisibility_and_divide_by_pow10_2(uint64_t *n) {
    // Make sure the computation for max_n does not overflow.
    const uint_fast32_t prod = *n * 656;

    const uint_fast32_t mask = (1UL << 16) - 1;
    bool const result = ((prod & mask) < 656);
...
// to even
static inline bool prefer_round_down(uint64_t significand) {
    return significand % 2 != 0;
}

static inline decimal_fp compute_nearest(signed_significand_bits s, exponent_int exponent_bits) {
    uint64_t two_fc = remove_sign_bit_and_shift(s);
    exponent_int binary_exponent = exponent_bits;

    // Is the input a normal number?
    if (binary_exponent != 0) {
        binary_exponent += ieee754_binary64.exponent_bias - ieee754_binary64.significand_bits;
        ...
        if (two_fc == 0) {
            // Compute k and beta.
            const int minus_k = floor_log10_pow2_minus_log10_4_over_3(binary_exponent);
            const int beta = binary_exponent + floor_log2_pow10(-minus_k);

            // Compute xi and zi.
            const uint128 cache = get_cache(-minus_k);

            uint64_t xi = compute_left_endpoint_for_shorter_interval_case(&cache, beta);
            uint64_t zi = compute_right_endpoint_for_shorter_interval_case(&cache, beta);
            ...
            uint64_t decimal_significand = divide_by_pow10_1_u64_32bit(zi, ((((1UL << (52 + 1)) + 1) / 3) + 1) * 20);
    ...
    const builtin_uint128_t cache = get_cache(-minus_k);
    const int beta = binary_exponent + floor_log2_pow10(-minus_k);

    // Compute zi and deltai.
    // 10^kappa <= deltai < 10^(kappa + 1)
    const uint64_t deltai = compute_delta(&cache, beta);
    ...
    // fine.
    const compute_mul_result z_result = compute_mul((uint64_t)((two_fc | 1) << beta), &cache);
    ...
    const uint64_t big_divisor = compute_power_3_u64(10);
    const uint64_t small_divisor = compute_power_2_u64(10);

    // Using an upper bound on zi, we might be able to optimize the division
    // better than the compiler; we are computing zi / big_divisor here.
    uint64_t decimal_significand = divide_by_pow10_3_u64_32bit(z_result.integer_part, (1UL << (52 + 1)) * big_divisor - 1);
    uint64_t r = z_result.integer_part - big_divisor * decimal_significand;

    do {
        if (r < deltai) {
            // Exclude the right endpoint if necessary.
            if ((r | (uint64_t)(!z_result.is_integer) |
                    (uint64_t)(true)) == 0) {
                    --decimal_significand;
                    r = big_divisor;
                    break;
            ...
            const compute_mul_parity_result x_result = compute_mul_parity(two_fc - 1, &cache, beta);

            if (!(x_result.parity |
                    (x_result.is_integer & true))) {
                break;
            }
        }
    ...
    // be larger than r.
    uint64_t dist = r - (deltai / 2) + (small_divisor / 2);
    const bool approx_y_parity = ((dist ^ (small_divisor / 2)) & 1) != 0;

    // Is dist divisible by 10^kappa?
    bool const divisible_by_small_divisor = check_divisibility_and_divide_by_pow10_2(dist);
    ...
    if (divisible_by_small_divisor) {
        ...
        // is an even number.
        const compute_mul_parity_result y_result = compute_mul_parity(two_fc, &cache, beta);
    ...
    return (decimal_fp){
        .significand = decimal_significand,
        .exponent = minus_k + kappa
    };
}

static inline decimal_fp compute_left_closed_directed(signed_significand_bits s,
                                        exponent_int exponent_bits) {

        uint64_t two_fc = remove_sign_bit_and_shift(s);
        int binary_exponent = exponent_bits;

        // Is the input a normal number?
        if (binary_exponent != 0) {
                        binary_exponent += -1023 - 52;
                        two_fc |= (1UL << (52 + 1));
                    }
        // Is the input a subnormal number?
        else {
        ...
        // 10^kappa <= deltai < 10^(kappa + 1)
        const uint64_t deltai = compute_delta(&cache, beta);
        compute_mul_result x_result = compute_mul(two_fc << beta, &cache);
        ...
        uint64_t decimal_significand = divide_by_pow10_3_u64_32bit(
            x_result.integer_part, (1UL << (52 + 1)) * big_divisor - 1);
        uint64_t r = x_result.integer_part - big_divisor * decimal_significand;
        ...
        do {
            if (r > deltai) {
                ...
                // an integer.
                compute_mul_parity_result const z_result = compute_mul_parity(two_fc + 2, &cache, beta);
        ...
        decimal_significand *= 10;
        decimal_significand -= small_division_by_pow10_2_u64(r);
        return (decimal_fp){
            .significand = decimal_significand,
            .exponent = minus_k + kappa
        };
...
static inline decimal_fp compute_right_closed_directed(signed_significand_bits s, exponent_int exponent_bits) {
    uint64_t two_fc = remove_sign_bit_and_shift(s);
    int binary_exponent = exponent_bits;
    ...
    // Compute zi and deltai.
    // 10^kappa <= deltai < 10^(kappa + 1)
    const uint64_t deltai = compute_delta(&cache, beta - (shorter_interval ? 1 : 0));
    uint64_t const zi = compute_mul(two_fc << beta, &cache).integer_part;
    ...
    uint64_t decimal_significand = divide_by_pow10_3_u64_32bit(zi, (1UL << (52 + 1)) * big_divisor - 1);
    const uint64_t r = zi - big_divisor * decimal_significand;
...
static inline bool is_right_endpoint_integer_shorter_interval(exponent_int binary_exponent) {
...
static inline bool is_left_endpoint_integer_shorter_interval(exponent_int binary_exponent) {
...
decimal_fp dragonbox_to_decimal(double x) {
    const float_bits br = make_float_bits(x);
    const int exponent_bits = extract_exponent_bits(br);
    const uint64_t s = remove_exponent_bits(br);
