...
static inline uint128 *uint128_add(uint128 *v, uint64_t n) {
    uint64_t sum = (v->low + n) & UINT64_C(0xffffffffffffffff);
    v->high += (sum < v->low ? 1 : 0);
    v->low = sum;
    return v;
}

static inline uint64_t umul64(uint32_t x, uint32_t y) { return x * (uint64_t)(y); }
...
static inline uint128 umul128(uint64_t x, uint64_t y) {
    ...
    uint64_t const ad = umul64(a, d);
    uint64_t const bd = umul64(b, d);

    uint64_t const intermediate = (bd >> 32) + (uint32_t)(ad) + (uint32_t)(bc);

    return (uint128){.high = ac + (intermediate >> 32) + (ad >> 32) + (bc >> 32),
                     .low = (intermediate << 32) + (uint32_t)(bd)};
}
...
// unsigned integer.
static inline uint128 umul192_upper128(uint64_t x, uint128 y) {
    uint128 r = umul128(x, y.high);
    uint128_add(&r, umul128_upper64(x, y.low));
    return r;
}

// Get upper 64-bits of multiplication of a 32-bit unsigned integer and a 64-bit
// unsigned integer.
static inline uint64_t umul96_upper64(uint32_t x, uint64_t y) {
    uint32_t const yh = y >> 32;
    uint32_t const yl = y;

    uint64_t const xyh = umul64(x, yh);
    uint64_t const xyl = umul64(x, yl);
...
// unsigned integer.
static inline uint128 umul192_lower128(uint64_t x, uint128 y) {
    uint64_t const high = x * y.high;
    uint128 const high_low = umul128(x, y.low);
    return (uint128){.high = (high + high_low.high) & UINT64_C(0xffffffffffffffff),
                     .low = high_low.low};
}
...
typedef struct {
    ...
    bool is_integer;
} compute_mul_result_u32;

typedef struct {
    uint64_t integer_part;
    bool is_integer;
} compute_mul_result_u64;
...
static inline compute_mul_result_u64 compute_mul(uint64_t u, uint128 cache) {
    uint128 const r = umul192_upper128(u, cache);
    return (compute_mul_result_u64){.integer_part = r.high, .is_integer = r.low == 0};
}

static inline uint64_t compute_delta(uint128 cache, int beta) {
    return cache.high >> (total_bits - 1 - beta);
}

static inline compute_mul_parity_result compute_mul_parity(uint64_t two_f, uint128 cache,
                                                           int beta) {
    uint128 const r = umul192_lower128(two_f, cache);
    assert(beta >= 1);
    assert(beta < 64);
    return (compute_mul_parity_result){
        .is_integer = ((r.high >> (64 - beta)) & 1) != 0,
        .parity = (((r.high << beta) & UINT64_C(0xffffffffffffffff)) |
                   (r.low >> (64 - beta))) == 0};
}

static inline uint64_t compute_left_endpoint_for_shorter_interval_case(uint128 cache,
                                                                       int beta) {
    return (cache.high - (cache.high >> (significand_bits + 2))) >>
           (total_bits - significand_bits - 1 - beta);
}

static inline uint64_t compute_right_endpoint_for_shorter_interval_case(uint128 cache,
                                                                        int beta) {
    return (cache.high + (cache.high >> (significand_bits + 1))) >>
           (total_bits - significand_bits - 1 - beta);
}

static inline uint64_t compute_round_up_for_shorter_interval_case(uint128 cache,
                                                                  int beta) {
...
static inline binary_fp decompose_float(double x) {
    uint64_t bits;
...
static inline bool is_finite(int binary_exponent) {
    return binary_exponent != (1u << exponent_bits) - 1;
}

// The main algorithm assumes the input is a normal/subnormal finite number.
static inline decimal_fp to_decimal_intern(uint64_t binary_significand,
                                           int binary_exponent) {
    bool is_even = binary_significand % 2 == 0;
    uint64_t two_fc = binary_significand * 2;
    ...
    if (binary_exponent != 0) {
        ...
        if (two_fc == 0) {
            ...
            // Compute xi and zi.
            uint128 const cache = cache_table[-minus_k - min_k];
            ...
            decimal_significand = divide_by_pow10(
                1, (((UINT64_C(2) << significand_bits) + 1) / 3 + 1) * 20, zi);
    ...
    //////////////////////////////////////////////////////////////////////
    {
        ...
        uint64_t const deltai = compute_delta(cache, beta);
        ...
        // fine.
        compute_mul_result_u64 const z_result =
            compute_mul((uint64_t)((two_fc | 1) << beta), cache);
        ...
        uint64_t decimal_significand =
            divide_by_pow10(kappa + 1, (UINT64_C(2) << significand_bits) * big_divisor - 1,
                            z_result.integer_part);
        uint64_t r = (uint64_t)(z_result.integer_part - big_divisor * decimal_significand);

        do {
            if (r < deltai) {
                // Exclude the right endpoint if necessary.
                if ((r | (uint64_t)(!z_result.is_integer) | (uint64_t)(is_even)) == 0) {
                    --decimal_significand;
                    r = big_divisor;
                    break;
                }
                ...
                compute_mul_parity_result const x_result =
                    compute_mul_parity((uint64_t)(two_fc - 1), cache, beta);
        ...
        decimal_significand *= 10;
        {
            ...
            // be larger than r.
            uint64_t dist = (uint64_t)(r - (deltai / 2) + (small_divisor / 2));
            ...
            if (divisible_by_small_divisor) {
                ...
                // is an even number.
                compute_mul_parity_result const y_result =
...
decimal_fp dragonbox_to_decimal(double x) {
    ...
    assert(is_finite(decomposed.exponent) &&
           (decomposed.significand != 0 || decomposed.exponent != 0));

    return to_decimal_intern(decomposed.significand, decomposed.exponent);
}
